...
import requests
import json
from my_journalistic_crew.models.outputs import SerperWebSearchResults, GoogleImageResults, GoogleNewsResults

class SerperDevToolInput(BaseModel):
    """Input schema for SerperDevTool."""
    query: str = Field(..., description="The search query.")
    country: str = Field(default="us", description="The country code for the search (e.g., 'us' for United States).")
    ...
    date_range: str = Field(default="any_time", description="The date range for the search (e.g., 'any_time').")
...
class SerperDevTool(BaseTool):
    ...
    description: str = (
        "A tool for performing searches using the Serper API. Supports various search types including search, images, videos, places, maps, reviews, news, shopping, image search (lens), scholar, patent, and autocomplete."
    )
    args_schema: Type[BaseModel] = SerperDevToolInput

    def _run(self, query: str, country: str = "us", location: str = "", language: str = "en", date_range: str = "any_time", autocorrect: bool = True, results_num: int = 10, page: int = 1, search_type: str = "search", tbs: str = "") -> Union[SerperWebSearchResults, GoogleImageResults, GoogleNewsResults]:
        ...
        if search_type == "images":
            ...
            return GoogleNewsResults(**search_results)
        else:
